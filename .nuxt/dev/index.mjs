    ...
    "routeRules": {
      ...
      "/sitemap.xml": {
        "headers": {
          ...
          "X-Sitemap-Prerendered": "2025-11-07T06:05:46.182Z"
        }
      },
      "/_nuxt/builds/meta/**": {
        "headers": {
...
const search_get = defineEventHandler(async (event) => {
  const posts = await serverQueryContent(event, "posts").where({ _draft: { $ne: true } }).only(["_path", "title", "excerpt", "date", "tags", "hero", "readingTime"]).sort({ date: -1 }).find();
  return posts;
});
...
const sitemapUrls_get = defineEventHandler(async (event) => {
  ...
  const posts = await serverQueryContent(event, "posts").where({ _draft: { $ne: true } }).only(["_path", "date", "tags"]).find();
  const paths = /* @__PURE__ */ new Set(["/", "/about", "/search"]);
  const tags = /* @__PURE__ */ new Set();
  for (const p of posts) {
    if (p == null ? void 0 : p._path) paths.add(p._path);
...
const sitemapUrls_get$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
